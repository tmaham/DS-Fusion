...
class LatentDiffusion(DDPM):
    ...
    @torch.no_grad()
    def log_view(self, batch):
        base_count = 0
        with self.ema_scope():
            uc = self.get_learned_conditioning(3 * [""])
            cond = self.get_learned_conditioning(3 * batch["cond"])
            sampler = DDIMSampler(self)
            
            for i in range(3):
                # samples, z_denoise_row = self.sample_log(cond=cond,batch_size=3,ddim=True,ddim_steps=50,eta=1.0,uc=uc)
                shape = [4, 32, 32]
    ...
    def sample_log(self,cond,batch_size,ddim, ddim_steps,uc=None,**kwargs):
        ...
        if ddim:
            ...
            samples, intermediates =ddim_sampler.sample(ddim_steps,batch_size,
                                                        shape,cond,uc=uc,verbose=False,**kwargs)
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=50, ddim_eta=1., return_keys=None,
        ...
        if sample:
            ...
            # save_image(sample_scaled[0], "visualization/output"+str(self.iter)+".png")
